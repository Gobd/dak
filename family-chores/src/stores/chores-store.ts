...
export const useChoresStore = create<ChoresState>((set, get) => ({
  ...
  fetchChores: async () => {
    ...
    const choresWithAssignments: ChoreWithAssignments[] = chores.map((chore) => ({
      ...chore,
      assignments: (assignments ?? [])
        .filter((a) => a.chore_id === chore.id)
        .map((a) => ({
          id: a.id,
          member: a.family_members as unknown as ChoreWithAssignments['assignments'][0]['member'],
        })),
    }));
  ...
  addChore: async (data, assigneeIds) => {
    const { data: newChore } = await supabase.from('chores').insert(data).select().single();
  ...
  toggleActive: async (id) => {
    ...
    await supabase.from('chores').update({ is_active: !chore.is_active }).eq('id', id);
