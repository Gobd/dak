  ...
  return (
    <div className="bg-neutral-800 rounded-lg p-3 w-[280px]">
      ...
      <div className="grid grid-cols-7 gap-1 mb-1">
        {DAY_NAMES.map((name) => (
          <div key={name} className="text-center text-xs text-neutral-500 py-1">
            {name}
          </div>
        ))}
      </div>
          ...
          return (
            <button
              ...
              className={`w-8 h-8 text-sm rounded-full flex items-center justify-center transition-colors
                ...
                ${!isSelected ? 'hover:bg-neutral-700' : ''}`}
            >
              {day}
            </button>
          );
...
export function DatePickerCompact({ value, onChange }: DatePickerProps) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-black rounded-xl p-4 shadow-2xl">
            ...
            <button
              ...
              type="button"
              className="w-full mt-3 px-4 py-2 rounded-lg bg-neutral-700 text-white font-medium"
            >
              Cancel
            </button>
          </div>
