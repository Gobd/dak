 ...
 */
export function getPosition(dateValue, lat, lng) {
...
const suntimesDeprecated = timesDeprecated;
 ...
 */
export function addTime(angleAltitude, riseName, setName, risePos, setPos, degree) {
  let isValid =
 ...
 */
export function addDeprecatedTimeName(alternameName, originalName) {
  let isValid =
 ...
 */
export function getSunTimes(dateValue, lat, lng, height, addDeprecated, inUTC) {
 ...
 */
export function getSunTime(dateValue, lat, lng, elevationAngle, height, degree, inUTC) {
 ...
 */
export function getSunTimeByAzimuth(dateValue, lat, lng, nazimuth, degree) {
  if (isNaN(nazimuth)) {
 ...
 */
export function getSolarTime(dateValue, lng, utcOffset) {
  // @ts-ignore
  const date = new Date(dateValue);
 ...
 */
export function getMoonPosition(dateValue, lat, lng) {
...
const fractionOfTheMoonCycle = moonCycles;
 ...
 */
export function getMoonIllumination(dateValue) {
 ...
 */
export function getMoonData(dateValue, lat, lng) {
 ...
 */
export function getMoonTimes(dateValue, lat, lng, inUTC) {
  if (isNaN(lat)) {
    throw new Error('latitude missing');
  }
  if (isNaN(lng)) {
 ...
 */
export function moonTransit(rise, set, lat, lng) {
  ...
  if (rise) {
    tempTransitAfter = calcMoonTransit(
      riseValue,
      getMoonTimes(new Date(riseDate).setDate(day + 1), lat, lng).set.valueOf()
    );
  ...
  if (set) {
    tempTransitBefore = calcMoonTransit(
      setValue,
      getMoonTimes(new Date(setDate).setDate(day - 1), lat, lng).rise.valueOf()
    );
